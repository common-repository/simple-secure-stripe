 ...
 */
namespace SimpleSecureWP\SimpleSecureStripe\StellarWP\Dates;
...
class Timezones {
	...
	public static function abbr( $date, $timezone_string ) {
		try {
			...
			|| ( class_exists( 'DateTimeImmutable' ) && $date instanceof DateTimeImmutable )
				? $date
				: Dates::build_date_object( $date, $timezone_object );
	...
	public static function apply_offset( $datetime, $offset, $invert = false ) {
		...
		if ( $offset_datetime && $offset_datetime->modify( $offset ) ) {
			return $offset_datetime->format( Dates::DBDATETIMEFORMAT );
		}
	...
	public static function clear_site_timezone_abbr( $option_val ) {
		delete_transient( 'stellarwp/dates/wp_timezone_abbr' );
		return $option_val;
	}
	...
	public static function maybe_get_tz_name( $timezone ) {
		...
		$timezone = timezone_name_from_abbr( '', $offset * 3600, 1 );
		...
		if ( false === $timezone ) {
			$timezone = timezone_name_from_abbr( '', (int) $offset * 3600, 1 );
		}
	...
	public static function mode() {
		...
		return apply_filters( 'stellarwp/dates/current_display_timezone', $mode );
	}
	...
	public static function to_tz( $datetime, $tzstring ) {
		...
		if ( $new_datetime ) {
			$new_datetime->setTimezone( $local );
			return $new_datetime->format( Dates::DBDATETIMEFORMAT );
		}
	...
	public static function to_utc( $datetime, $tzstring, $format = null ) {
		...
		if ( $new_datetime ) {
			...
			$format = ! empty( $format ) ? $format : Dates::DBDATETIMEFORMAT;
