...
abstract class Util
{
    ...
    {
        $types = \SimpleSecureWP\SimpleSecureStripe\Stripe\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \SimpleSecureWP\SimpleSecureStripe\Stripe\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \SimpleSecureWP\SimpleSecureStripe\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
