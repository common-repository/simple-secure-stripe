import './style.scss';
import {registerCreditCardForm} from "@simple-secure-stripe/stripe/util";
import {CardNumberElement, CardExpiryElement, CardCvcElement} from '@stripe/react-stripe-js';
import {__} from "@wordpress/i18n";

const Bootstrap = ({CardIcon, options, onChange}) => {
    return (
        <div className='sswps-bootstrap-form'>
            <div className='row'>
                <div className='col-md-6 mb-3'>
                    ...
                    <label htmlFor="stripe-card-number">{__('Card Number', 'simple-secure-stripe')}</label>
                    {CardIcon}
                </div>
                <div className='col-md-3 mb-3'>
                    ...
                    <label htmlFor="stripe-exp">{__('Exp', 'simple-secure-stripe')}</label>
                </div>
                <div className='col-md-3 mb-3'>
                    <CardCvcElement className="md-form md-outline stripe-input" options={options['cardCvc']}
                                    onChange={onChange(CardCvcElement)}/>
                    <label htmlFor="stripe-cvv">{__('CVV', 'simple-secure-stripe')}</label>
                </div>
            </div>
        </div>
    )
