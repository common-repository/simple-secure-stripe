...
const CustomCardForm = (
    ...
    if (!CardForm) {
        return (
            <div className='sswps-custom-form-error'>

                <p>{
                /* translators: %s: custom form label. */
                sprintf(__('%s is not a valid blocks Stripe custom form. Please choose another custom form option in the Credit Card Settings.', 'simple-secure-stripe'), getData('customFormLabels')[id])
                }</p>
            </div>
        )
    }
    return (
        <div className={`sswps-custom-form ${id}`} ref={setContainer}>
...
const CardIcon = ({type, src}) => {
    if (type) {
        return <img className={`sswps-card ${type}`} src={src}/>
    }
    return null;
}
