...
namespace SimpleSecureWP\SimpleSecureStripe\Integrations\WooCommerceBlocks\Assets;
...
class Api {
	...
	protected $commons_script_name = 'sswps-blocks-commons';

	protected $commons_styles_name = 'sswps-block-style';
	...
	private function init() {
		foreach ( [ 'cart', 'checkout' ] as $page ) {
			add_action( "woocommerce_blocks_enqueue_${page}_block_scripts_after", [ $this, 'enqueue_style' ] );
		}
	...
	public function register_script( $handle, $relative_path, $deps = [] ) {
		...
		$default_deps = [ $this->commons_script_name ];
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( $handle, 'simple-secure-stripe' );
		}
	}
	...
	public function enqueue_style() {
		...
		if ( ! in_array( $this->commons_styles_name, $this->styles ) ) {
			wp_enqueue_style( $this->commons_styles_name, $this->get_asset_url( 'integrations/WooCommerceBlocks/woocommerceblocks-style.css' ), [], $this->config->get_version() );
			wp_style_add_data( $this->commons_styles_name, 'rtl', 'replace' );
			$this->styles[] = $this->commons_styles_name;

			do_action( 'sswps/blocks_enqueue_styles', $this );
		}
	}
