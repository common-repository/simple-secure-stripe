...
class Post_Request_Type {
	...
	public function is_inline_editing_post( $post_type ): bool {
		if ( ! ( ! empty( $post_type ) && wp_doing_ajax() && sswps_get_request_var( 'action' ) === 'inline-save' ) ) {
			return false;
		}

		$post_id = sswps_get_request_var( 'post_ID', null );
	...
	public function is_editing_post( $post_or_type = null ): bool {
		...
		if ( ! empty( $post_or_type ) ) {
			$current_post = Request::get_sanitized_var( 'post', get_post() );
			...
			if ( $is_post && $post instanceof WP_Post ) {
				...
				$post_type = Request::get_sanitized_var( 'post_type', 'post' );
			}
