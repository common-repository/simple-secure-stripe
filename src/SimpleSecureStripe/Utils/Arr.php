...
class Arr {
	 ...
	 *
	 * @param array        $array The array containing the key this sets.
	...
	public static function map_or_discard( $keys, array $map, &$found = true ) {
		...
		foreach ( (array) $keys as $key ) {
			$meta_key = static::get( $map, $key, $hash );
			if ( $hash === $meta_key ) {
				continue;
			}
			$mapped[] = $meta_key;
	 ...
	 * @since 1.0.0
	 *
	 * @param array $array     The array whose keys should be duplicated.
	 ...
	 */
	public static function add_unprefixed_keys_to( array $array, bool $recursive = false ) {
		$unprefixed = [];
	 ...
	 *               original input if not an array.
	 */
	public static function add_prefixed_keys_to( array $array, $recursive = false ) {
		$prefixed = [];
		foreach ( $array as $key => $value ) {
			if ( $recursive && is_array( $value ) ) {
