 ...
 *
 */
abstract class Abstract_Token extends \WC_Payment_Token {
	...
	public function get_payment_method_title( $format = '' ) {
		...
		return apply_filters( 'sswps/payment_method_title', str_replace( array_keys( $data ), $data, $format ), $this );
	}
	...
	public function get_stripe_data( $instance ) {
		...
		try {
			...
			if ( is_subclass_of( get_parent_class( $instance ), Abstract_Token::class ) ) {
	...
	public function to_json() {
		return apply_filters(
			'sswps_get_' . $this->object_type . '_json',
			[
				'id'      => $this->get_id(),
				'type'    => $this->get_type(),
	...
	public static function token_exists( $token_id, $user_id ) {
		global $wpdb;
		$count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}woocommerce_payment_tokens WHERE token = %s AND gateway_id LIKE %s AND user_id = %d", $token_id, '%sswps_%', $user_id ) );
