...
use SimpleSecureWP\SimpleSecureStripe\Utils\Paths;
use SimpleSecureWP\SimpleSecureStripe\Utils\Strings;
...
class Template {
	...
	/**
	 * The namespace for the template.
	 *
	 * @since 1.0.0
	 *
	 * @var  string
	 ...
	 * @since 1.0.0
	 *
	 * @param string $origin The base origin class for the templates.
	 *
	 * @return self
	 */
	public function set_template_origin( string $origin = null ) {
		if ( empty( $origin ) ) {
			$origin = $this->origin;
		}
		...
		$this->origin             = $origin;
		$this->template_base_path = untrailingslashit( $this->origin->plugin_path );
	 ...
	 * @return self
	 */
	public function set_template_folder_lookup( $value = true ) : Template {
		$this->template_folder_lookup = sswps_is_truthy( $value );
	 ...
	 */
	public function get_template_folder_lookup() : bool {
	 ...
	 * @return self
	 */
	public function add_template_globals( array $context = [] ) : Template {
	 ...
	 * @return self
	 */
	public function set_template_context_extract( bool $value = false ) : Template {
		// Cast as bool and save
		$this->template_context_extract = sswps_is_truthy( $value );
	 ...
	 */
	public function set_template_current_hook_name( $value ) : Template {
	...
	final public function get( $index, $default = null, $is_local = true ) {
		 ...
		 */
		$value = apply_filters( 'sswps/template/context_get', null, $index, $default, $is_local, $this );
	...
	public function merge_context( $context = [], $file = null, $name = null ) {
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$this->context = apply_filters( 'sswps/template/context', $context, $file, $name, $this );
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$this->context = apply_filters( "sswps/template/context:{$hook_name}", $this->context, $file, $name, $this );
	...
	protected function get_template_plugin_path() {
		 ...
		 * @param self   $template  Current instance of the Template
		 */
		return apply_filters( 'sswps/template/plugin_path', $path, $this );
	}
	...
	protected function get_template_public_namespace( $plugin_namespace ) {
		...
		} elseif ( ! empty( $this->template_namespace ) ) {
		 ...
		 * @param self   $template  Current instance of the Template
		 */
		return apply_filters( 'sswps/template/public_namespace', $namespace, $this );
	}
	...
	public function get_template_origin_base_folder() {
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		return apply_filters( 'sswps/template/origin_base_folder', $this->template_origin_base_folder, $this );
	}
	...
	protected function get_template_public_path( $base, $namespace ) {
		 ...
		 * @param self   $template  Current instance of the Template
		 */
		return apply_filters( 'sswps/template/public_path', $path, $this );
	}
	...
	protected function get_template_path_list() {
		 ...
		 * @param  self   $template  Current instance of the Template
		 */
		$folders = (array) apply_filters( 'sswps/template/path_list', $folders, $this );

		uasort( $folders, 'sswps_sort_by_priority' );
	...
	protected function get_template_theme_path_list( $namespace ) {
		 ...
		 * @param  self    $template    Current instance of the Template.
		 */
		$folders = (array) apply_filters( 'sswps/template/theme_path_list', $folders, $namespace, $this );
	...
	public function get_template_file( $name ) {
		...
		if ( $found_file ) {
			 ...
			 * @param self   $template  Current instance of the Template
			 */
			return apply_filters( 'sswps/template/file', $found_file, $name, $this );
		}
	...
	public function do_entry_point( $entry_point_name, $echo = true ) {
		 ...
		 */
		$is_entry_point_enabled = apply_filters( 'sswps/template/entry_point_is_enabled', true, $hook_name, $entry_point_name, $this );
		...
		if ( has_action( "sswps/template/entry_point:{$hook_name}" ) ) {
			/**
			 ...
			 * @param self   $template         Current instance of the template class doing this entry point.
			 */
			do_action( "sswps/template/entry_point:{$hook_name}", $hook_name, $entry_point_name, $this );
		}

		if ( has_action( "sswps/template/entry_point:{$hook_name}:{$entry_point_name}" ) ) {
			/**
			 ...
			 * @param self   $template         Current instance of the template class doing this entry point.
			 */
			do_action( "sswps/template/entry_point:{$hook_name}:{$entry_point_name}", $hook_name, $entry_point_name, $this );
		}

		$html = ob_get_clean();

		if ( has_filter( "sswps/template/entry_point_html:{$hook_name}" ) ) {
			/**
			 * Generic entry point action for the current template.
			 *
			 * @since 1.0.0
			 ...
			 * @param self   $template         Current instance of the template class doing this entry point.
			 */
			$html = apply_filters( "sswps/template/entry_point_html:{$hook_name}", $html, $hook_name, $entry_point_name, $this );
		}

		if ( has_filter( "sswps/template/entry_point_html:{$hook_name}:{$entry_point_name}" ) ) {
			/**
			 * Specific named entry point action called.
			 *
			 * @since 1.0.0
			 ...
			 * @param self   $template         Current instance of the template class doing this entry point.
			 */
			$html = apply_filters( "sswps/template/entry_point_html:{$hook_name}:{$entry_point_name}", $html, $hook_name, $entry_point_name, $this );
		}
	...
	public function template( $name, $context = [], $echo = true ) {
		 ...
		 * @param boolean $echo    If we should also print the Template.
		 */
		$done = apply_filters( 'sswps/template/done', null, $name, $context, $echo );
		 ...
		 * @param self   $template Current instance of the Template
		 */
		$pre_html = apply_filters( 'sswps/template/pre_html', null, $file, $name, $this );

		/**
		 * Allow users to filter the HTML by the name before rendering
		 *
		 * E.g.:
		 *    `sswps/template/pre_html:events/blocks/parts/details`
		 *    `sswps/template/pre_html:events/embed`
		 *    `sswps/template/pre_html:tickets/login-to-purchase`
		 *
		 * @since 1.0.0
		 *
		 ...
		 * @param self   $template  Current instance of the Template
		 */
		$pre_html = apply_filters( "sswps/template/pre_html:{$hook_name}", $pre_html, $file, $name, $this );
	...
	public function template_get_origin_namespace( $path ) {
		 ...
		 */
		$namespace_map = (array) apply_filters( 'sswps/template/origin_namespace_map', [], $path, $this );
		...
		if ( empty( $matching_namespace ) && ! empty( $this->template_namespace ) ) {
			$matching_namespace = $this->template_namespace;
		}
	...
	protected function filter_template_html( $html, $file, $name, $hook_name ) {
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( 'sswps/template/html', $html, $file, $name, $this );

		/**
		 * Allow users to filter the final HTML by the name.
		 *
		 * E.g.:
		 *    `sswps/template/html:events/blocks/parts/details`
		 *    `sswps/template/html:events/embed`
		 *    `sswps/template/html:tickets/login-to-purchase`
		 *
		 * @since 1.0.0
		 *
		 * @param string $html      The final HTML.
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( "sswps/template/html:{$hook_name}", $html, $file, $name, $this );
	...
	protected function filter_template_before_include_html( $html, $file, $name, $hook_name ) {
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( 'sswps/template/before_include_html', $html, $file, $name, $this );

		/**
		 * Allow users to filter the Before include actions by name.
		 *
		 * E.g.:
		 *    `sswps/template/before_include_html:events/blocks/parts/details`
		 *    `sswps/template/before_include_html:events/embed`
		 *    `sswps/template/before_include_html:tickets/login-to-purchase`
		 *
		 * @since 1.0.0
		 *
		 * @param string $html      The final HTML.
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( "sswps/template/before_include_html:{$hook_name}", $html, $file, $name, $this );
	...
	protected function filter_template_include_html( $html, $file, $name, $hook_name ) {
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( 'sswps/template/include_html', $html, $file, $name, $this );

		/**
		 * Allow users to filter the PHP template include actions by name.
		 *
		 * E.g.:
		 *    `sswps/template/include_html:events/blocks/parts/details`
		 *    `sswps/template/include_html:events/embed`
		 *    `sswps/template/include_html:tickets/login-to-purchase`
		 *
		 * @since 1.0.0
		 *
		 * @param string $html      The final HTML.
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( "sswps/template/include_html:{$hook_name}", $html, $file, $name, $this );
	...
	protected function filter_template_after_include_html( $html, $file, $name, $hook_name ) {
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( 'sswps/template/after_include_html', $html, $file, $name, $this );

		/**
		 * Allow users to filter the after include actions by name.
		 *
		 * E.g.:
		 *    `sswps/template/after_include_html:events/blocks/parts/details`
		 *    `sswps/template/after_include_html:events/embed`
		 *    `sswps/template/after_include_html:tickets/login-to-purchase`
		 *
		 * @since 1.0.0
		 *
		 * @param string $html      The final HTML.
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		$html = apply_filters( "sswps/template/after_include_html:{$hook_name}", $html, $file, $name, $this );
	...
	protected function actions_before_template( $file, $name, $hook_name ) {
		 ...
		 * @param self   $template  Current instance of the Template
		 */
		do_action( 'sswps/template/before_include', $file, $name, $this );

		/**
		 * Fires an Action for a given template name before including the template file,
		 *
		 * E.g.:
		 *    `sswps/template/before_include:events/blocks/parts/details`
		 *    `sswps/template/before_include:events/embed`
		 *    `sswps/template/before_include:tickets/login-to-purchase`
		 *
		 * @since 1.0.0
		 *
		 * @param string $file      Complete path to include the PHP File.
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		do_action( "sswps/template/before_include:{$hook_name}", $file, $name, $this );
	...
	protected function actions_after_template( $file, $name, $hook_name ) {
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		do_action( 'sswps/template/after_include', $file, $name, $this );

		/**
		 * Fires an Action for a given template name after including the template file.
		 *
		 * E.g.:
		 *    `sswps/template/after_include:events/blocks/parts/details`
		 *    `sswps/template/after_include:events/embed`
		 *    `sswps/template/after_include:tickets/login-to-purchase`
		 *
		 * @since 1.0.0
		 *
		 * @param string $file      Complete path to include the PHP File.
		 ...
		 * @param self   $template  Current instance of the Template.
		 */
		do_action( "sswps/template/after_include:{$hook_name}", $file, $name, $this );
