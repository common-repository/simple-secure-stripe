...
class API {
	...
	public function __construct( $base_path = null, $assets_url = null ) {
		$this->base_path  = $base_path ?? SIMPLESECUREWP_STRIPE_FILE_PATH;
		$this->assets_url = $assets_url ?? trailingslashit( plugins_url( dirname( SIMPLESECUREWP_STRIPE_BASENAME ) ) );
	...
	public function register_script( $handle, $relative_path, $deps = [], $version = null, $footer = true ) {
		...
		$deps = apply_filters( 'sswps/script_dependencies', $deps, $handle );
	...
	public function enqueue( $assets_to_enqueue = null, bool $should_enqueue_no_matter_what = false ) {
		...
		foreach ( $assets as $asset ) {
			 ...
			 */
			$enqueue = (bool) apply_filters( 'sswps/enqueue', $enqueue, $asset );
			 ...
			 * @param object $asset   Which asset we are dealing with.
			 */
			$enqueue = (bool) apply_filters( "sswps/enqueue_{$slug}", $enqueue, $asset );
	 ...
	 * @since 1.0.0
	 *
	 * @param Asset $asset Register an asset to the Asset API.
	 *
	 ...
	 */
	public static function asset( string $slug, string $file, string $version = Plugin::VERSION, $plugin_path = null ) {
		return App::get( static::class )->register( new Asset( $slug, $file, $version, $plugin_path ) );
	}
	...
	public function get( $slug = null, $sort = true ) {
		if ( is_null( $slug ) ) {
			if ( $sort ) {
				...
				// Sorts by priority.
				$cache_count = App::get_var( $cache_key_count, 0 );
				$count       = count( $this->assets );

				if ( $count !== $cache_count ) {
					uasort( $this->assets, static function ( $a, $b ) {
						return sswps_sort_by_priority( $a, $b, 'get_priority' );
					} );
					App::set_var( $cache_key_count, $count );
				}
			}
			return $this->assets;
		}
